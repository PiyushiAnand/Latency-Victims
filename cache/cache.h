...
extern std::unordered_map<uint8_t, std::unordered_map<uint8_t, HistoryTableEntry>> history_table;
...
class Cache : public CacheBase
{
   ...
   public:
      ...
      //for dead block predictor 
      bool hasExpired(CacheBlockInfo* block, SubsecondTime now,uint8_t pc,uint8_t block_addr);

      bool invalidateSingleLine(IntPtr addr);
      CacheBlockInfo* accessSingleLine(IntPtr addr,
            access_t access_type, Byte* buff, UInt32 bytes, SubsecondTime now, bool update_replacement, bool tlb_entry=false, bool is_metadata=false,IntPtr eip=0);
      ...
      void insertSingleLine(IntPtr addr, Byte* fill_buff,
            ...
            CacheBlockInfo* evict_block_info, Byte* evict_buff, SubsecondTime now, CacheCntlr *cntlr = NULL,  CacheBlockInfo::block_type_t btype = CacheBlockInfo::block_type_t::NON_PAGE_TABLE,IntPtr eip=0);
