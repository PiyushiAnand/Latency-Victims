      ...
      Core::mem_origin_t mem_origin)
{
   ...
   if (!cache_hit && m_perfect)
   {
      ...
      {
         insertCacheBlock(ca_address, mem_op_type == Core::READ ? CacheState::SHARED : CacheState::MODIFIED, NULL, m_core_id, ShmemPerfModel::_USER_THREAD, block_type,eip);
   ...
   if (cache_hit)
   {
      ...
      if( !(metadata_request && (metadata_passthrough_loc !=1)) )
      {
         ...
         copyDataFromNextLevel(mem_op_type, ca_address, modeled, t_now, block_type,eip);
...
void
CacheCntlr::copyDataFromNextLevel(Core::mem_op_t mem_op_type, IntPtr address, bool modeled, SubsecondTime t_now, CacheBlockInfo::block_type_t block_type,IntPtr eip)
{
...
MYLOG("copyDataFromNextLevel l%d", m_mem_component);
   ...
   else
   {
      ...
      insertCacheBlock(address, cstate, data_buf, m_core_id, ShmemPerfModel::_USER_THREAD, block_type,eip);
...
CacheCntlr::processShmemReqFromPrevCache(IntPtr eip, CacheCntlr* requester, Core::mem_op_t mem_op_type, IntPtr address, bool modeled, bool count,CacheBlockInfo::block_type_t block_type, Prefetch::prefetch_type_t isPrefetch, SubsecondTime t_issue, bool have_write_lock, Core::mem_origin_t mem_origin)
{
   ...
   if (!cache_hit && m_perfect)
   {
      ...
      if (cache_block_info)
         ...
         cache_block_info = insertCacheBlock(address, mem_op_type == Core::READ ? CacheState::SHARED : CacheState::MODIFIED, NULL, m_core_id, ShmemPerfModel::_USER_THREAD, block_type,eip);
   }
   ...
   else // !cache_hit: either data is not here, or operation on data is not permitted
   {
      ...
      if (m_next_cache_cntlr)
      {
         ...
         if (hit_where != HitWhere::MISS)
         {
            ...
            SubsecondTime t_now = getShmemPerfModel()->getElapsedTime(ShmemPerfModel::_USER_THREAD);
            copyDataFromNextLevel(mem_op_type, address, modeled, t_now,block_type,eip);
            if (isPrefetch != Prefetch::NONE)
      ...
      else // last-level cache
      {
         ...
         else if (m_master->m_dram_cntlr)
         {
            ...
            else
            {
               ...
               insertCacheBlock(address, mem_op_type == Core::READ ? CacheState::SHARED : CacheState::MODIFIED, data_buf, m_core_id, ShmemPerfModel::_USER_THREAD, block_type,eip);
               if (isPrefetch != Prefetch::NONE)
                  getCacheBlockInfo(address)->setOption(CacheBlockInfo::PREFETCH);
...
SharedCacheBlockInfo*
CacheCntlr::insertCacheBlock(IntPtr address, CacheState::cstate_t cstate, Byte* data_buf, core_id_t requester, ShmemPerfModel::Thread_t thread_num, CacheBlockInfo::block_type_t block_type,IntPtr eip)
{
   ...
   m_master->m_cache->insertSingleLine(address, data_buf,
         ...
         getShmemPerfModel()->getElapsedTime(thread_num), this, block_type,eip);
...
MYLOG("processExRepFromDramDirectory l%d", m_mem_component);
   ...
   insertCacheBlock(address, CacheState::EXCLUSIVE, data_buf, requester, ShmemPerfModel::_SIM_THREAD,shmem_msg->getBlockType(),0);
...
MYLOG("processShRepFromDramDirectory l%d", m_mem_component);
   ...
   // @RBERA: this is CacheBlockInfo::block_type_t leak
   insertCacheBlock(address, CacheState::SHARED, data_buf, requester, ShmemPerfModel::_SIM_THREAD,shmem_msg->getBlockType(),0);
}
