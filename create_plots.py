...
print("L2 metrics")
selected_metrics = [
    "pwc_L2.access", "pwc_L2.miss", 
    "pwc_L3.access", "pwc_L3.miss", 
    "pwc_L4.access", "pwc_L4.miss",
    'performance_model.cycle_count','performance_model.instruction_count'
]
...
# Populate the plot_data list with the relevant information for plotting
for experiment in selected_experiments:
    for metric in selected_metrics:
        value = df.loc[df['Exp'] == experiment, metric].values[0]  # assuming each experiment has one row
        
        plot_data.append({'experiment': experiment, 'metric': metric, 'value': value})

# # Convert the list into a DataFrame
plot_df = pd.DataFrame(plot_data)

# # Pivot the data to make it suitable for plotting
pivot_df = plot_df.pivot(index='experiment', columns='metric', values='value')

# # Plotting each metric as a bar plot
fig, ax = plt.subplots(figsize=(10, 6))

# # Plot the individual metrics
memory_metrics = ["pwc_L2.access", "pwc_L2.miss", 
    "pwc_L3.access", "pwc_L3.miss", 
    "pwc_L4.access", "pwc_L4.miss"]
pivot_df[memory_metrics].plot(kind='bar', ax=ax, width=0.8)
ax.set_xlabel('Experiment')
ax.set_ylabel('Metric Value')
ax.set_title('L2, L3, and L4 Cache Metrics for Different Experiments(Dead block)')
plt.grid(True)
...
# # Show the plot
plt.tight_layout()
plt.savefig('./plots/misses_n_accesses.png')


###plotting the L2 miss ratio
print("Access/Miss Ratio Across experiments (Dead block)")
fig, ax = plt.subplots(figsize=(10, 6))
pivot_df['L2_Miss_Ratio'] = pivot_df['pwc_L2.miss'] / pivot_df['pwc_L2.access']
pivot_df['L2_Miss_Ratio'].plot(kind='bar', ax=ax, width=0.8)
ax.set_xlabel('Experiment')
ax.set_ylabel('Miss Ratio')
ax.set_title('L2 Miss Ratio Different Experiments(Dead block)')
plt.grid(True)
ax.legend(title='Metrics', bbox_to_anchor=(1.05, 1), loc='upper left')
plt.savefig('./plots/miss_ratio.png')


##plot of ipc values
print('IPC values')

# Create a new column for IPC by dividing instruction count by cycle count
pivot_df['IPC'] = pivot_df['performance_model.instruction_count'] / pivot_df['performance_model.cycle_count']
fig, ax = plt.subplots(figsize=(10, 6))
pivot_df['IPC'].plot(kind='bar', ax=ax, width=0.8)
ax.set_xlabel('Experiment')
ax.set_ylabel('IPC')
ax.set_title('Instructions Per Cycle (IPC) for Different Experiments(Dead block)')
plt.grid(True)
ax.legend(title='Metrics', bbox_to_anchor=(1.05, 1), loc='upper left')
plt.tight_layout()
plt.savefig('./plots/ipc_values.png')
print(pivot_df['IPC'])
