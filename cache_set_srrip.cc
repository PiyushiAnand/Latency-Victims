...
CacheSetSRRIP::getReplacementIndex(CacheCntlr *cntlr)
{
   ...
   UInt32 dead_block_index = -1;
   UInt8 max_rrpv = 0; // Initialize the max RRIP value as 0
   ...
   for (UInt32 i = 0; i < m_associativity; i++)
   {
      ...
      if (!m_cache_block_info_array[i]->conf || m_cache_block_info_array[i]->ref_count < m_cache_block_info_array[i]->max_C_past) continue;

      // We have a dead block, now check if it has the highest RRIP value
      if (m_rrip_bits[i] > max_rrpv)
      {
         max_rrpv = m_rrip_bits[i];
         dead_block_index = i;
      }
   }

   if (dead_block_index != -1)
   {
      // If we found a dead block, replace it
      m_rrip_bits[dead_block_index] = m_rrip_insert; // Prepare the way for the new line
      return dead_block_index;
   }
